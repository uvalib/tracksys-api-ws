...
<xsl:stylesheet xmlns="http://www.loc.gov/mods/v3" xmlns:marc="http://www.loc.gov/MARC21/slim"
  ...
  <!-- This stylesheet is functionally the same as MARC21slim2MODS3-6_rev.xsl. It was 
    generated by resolveStylesheetIncludes.xsl (v. 0.1) 2023-03-01T15:45 -->
  ...
  <!-- This stylesheet is based on MARC21slim2MODS3-6, available at
    ...
    Even so, the output is MODS 3.8 compliant. -->
  ...
  <!-- IMPORTS/INCLUDES -->
  <!-- <xsl:import href="MARC21slimUtils_rev.xsl"/> -->
  ...
  <xsl:template match="/">
    <xsl:choose>
      ...
      <xsl:otherwise>
        <mods xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" version="3.8">
  ...
  <xsl:template name="marcRecord">
      ...
      <xsl:variable name="physDetails">
        ...
        <xsl:for-each select="*:datafield[@tag = '300']">
          <xsl:for-each select="*:subfield[@code = 'a']">
            ...
            <!-- Color content in $a -->
            <xsl:analyze-string select="$subfieldA" regex="(cols?\.?|colou?r(ed|s)?|b&amp;w)">
            ...
            <!-- $a after the 1st one assumed to be accompanying material -->
            <xsl:if test="position() &gt; 1">
        ...
        <xsl:for-each select="*:datafield[@tag = '300']/*:subfield[@code = 'b' or @code = 'e']">
          ...
          <!-- Illustrative content in $b or $e -->
          <xsl:analyze-string select="$subfieldBE"
            ...
            <xsl:matching-substring>
              <note type="{$notePrefix}illustrations">
                <!-- Replace singular term with its plural form -->
                <xsl:value-of select="
                    replace(replace(replace(replace(replace(replace(
                    replace(replace(replace(replace(replace(replace(replace(replace(
                    replace(replace(., 'charts?', 'charts'),
                    'coats? of arms', 'coats of arms'),
                    'diagr(am)?s?', 'diagrams'),
                    'facsim(ile)?s?\.?', 'facsimiles'),
                    'forms?', 'forms'),
                    'geneal(ogical)?\.? tables?', 'genealogical tables'),
                    'illuminations?', 'illuminations'),
                    'ill(u?s)?\.?trations?', 'illustrations'),
                    'lithographs?', 'lithographs'),
                    'maps?', 'maps'),
                    'musics?', 'music'),
                    'photo(graph)?s?\.?', 'photographs'),
                    'plans?', 'plans'),
                    'plates?', 'plates'),
                    'port(raits)?s?\.?', 'portraits'),
                    'samples?', 'samples')
                    "/>
              </note>
            </xsl:matching-substring>
          </xsl:analyze-string>
          <!-- Playback channel in $b or $e -->
          <xsl:analyze-string select="$subfieldBE"
    ...
    <recordInfo>
      ...
      <xsl:for-each select="*:datafield[@tag = '040']">
        <xsl:choose>
          <xsl:when test="*:subfield[@code = 'b'][normalize-space(.) != '']">
            <xsl:for-each select="*:subfield[@code = 'b'][normalize-space(.) != '']">
      ...
      <recordOrigin>Converted from MARCXML to MODS version 3.8 using <xsl:value-of
          ...
          select="format-time(current-time(), '[h]:[m] [P]')"/>
      </recordOrigin>
    </recordInfo>
  </xsl:template>
      ...
      <xsl:choose>
        <xsl:when test="$elementName = 'location'">
          <url displayLabel="electronic resource">
            ...
            <xsl:call-template name="createUrlAccess"/>
  ...
  <xsl:template name="createTitleInfoFrom246">
    <titleInfo type="alternative">
      ...
      <title>
        <xsl:call-template name="chopPunctuation">
          <xsl:with-param name="chopString">
            <xsl:value-of select="string-join(*:subfield[matches(@code, '[af]')], ' ')"/>
          </xsl:with-param>
        </xsl:call-template>
      </title>
      <xsl:call-template name="subtitle"/>
  ...
  <xsl:template name="createUrlAccess">
    <xsl:attribute name="access">
      <xsl:choose>
        <xsl:when test="*:subfield[@code = 'e'][matches(., 'UVA TrackSys')]">
          <xsl:text>object in context</xsl:text>
        </xsl:when>
        <xsl:when test="*:subfield[matches(@code, '[3uy]')][matches(., 'preview', 'i')]">
