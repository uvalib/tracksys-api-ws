...
<xsl:stylesheet xmlns="http://dplava.lib.virginia.edu"
  ...
  <!-- This stylesheet is functionally the same as uvaMAP2DPLA.xsl. It was 
    generated by resolveStylesheetIncludes.xsl (v. 0.1) 2023-11-15T13:15 -->
  ...
  <xsl:variable name="dplaGenreList">
    ...
    <genre valueURI="http://vocab.getty.edu/aat/300386879">ceramic ware (visual
      works)</genre>
    ...
    <genre valueURI="http://vocab.getty.edu/aat/300209275">costume (mode of
      fashion)</genre>
    ...
    <genre valueURI="http://vocab.getty.edu/aat/300136900">motion pictures (visual
      works)</genre>
    <genre valueURI="http://vocab.getty.edu/aat/300379591">natural history
      specimens</genre>
    <genre valueURI="http://vocab.getty.edu/aat/300026656">newspapers</genre>
    <genre valueURI="http://vocab.getty.edu/aat/300202595">oral histories (document
      genre)</genre>
    ...
    <genre valueURI="http://vocab.getty.edu/aat/300069200">performances (creative
      events)</genre>
    ...
    <genre valueURI="http://vocab.getty.edu/aat/300047090">sculpture (visual
      works)</genre>
  ...
  <xsl:variable name="runDateTime">
    <xsl:value-of
      select="format-dateTime(current-dateTime(), '[Y][M02][D02]T[H01][m][s]')"/>
  </xsl:variable>
  ...
  <xsl:template match="/">
    <xsl:processing-instruction name="xml-model"
      >href="https://dplava.lib.virginia.edu/dplava.xsd" type="application/xml"
  ...
  <xsl:template match="*:uvaMAP">
    <mdRecordGrp xsi:schemaLocation="http://dplava.lib.virginia.edu
  ...
  <xsl:template match="*:doc">
    <mdRecord>
      ...
      <xsl:choose>
        ...
        <!-- Original identifier -->
        <xsl:when
          test="*:field[matches(@name, '^(orig_identifier)$')][not(@invalid = 'yes')]">
          <xsl:apply-templates
        ...
        <xsl:when test="*:field[matches(@name, '^(sourceRecordIdentifier)$')]">
          <xsl:apply-templates
            select="*:field[matches(@name, '^(sourceRecordIdentifier)$')][1]"/>
        </xsl:when>
        <!-- OCLC number -->
      ...
      <xsl:variable name="otherTitles">
        ...
        <xsl:for-each
          select="*:field[matches(@name, 'work_title') and not(@supplied eq 'yes')]">
          <xsl:sort/>
          <dcterms:title>
            <xsl:value-of select="concat(upper-case(substring(., 1, 1)), substring(., 2))"
            />
      ...
      <xsl:variable name="agentsSorted">
        <xsl:for-each select="$agents/*:field">
          <xsl:sort/>
          <xsl:copy-of select="."/>
        </xsl:for-each>
      </xsl:variable>
      <xsl:variable name="agentsDeduped">
        <xsl:for-each select="$agentsSorted/*:field">
          <xsl:if test="not(. = preceding-sibling::*:field)">
            <xsl:apply-templates select="."/>
          </xsl:if>
        </xsl:for-each>
      </xsl:variable>
      <!-- Output creator(s) first -->
      <xsl:for-each select="$agentsDeduped/*:creator">
        <xsl:copy-of select="."/>
      </xsl:for-each>
      <xsl:for-each select="$agentsDeduped/*:contributor">
        <xsl:copy-of select="."/>
      </xsl:for-each>
      <!-- Publisher -->
      ...
      <xsl:choose>
        <xsl:when
          test="*:field[matches(@name, '^(dateIssued)$')][not(matches(., 'undated'))]">
          <xsl:apply-templates
            select="*:field[matches(@name, '^(dateIssued)$')][not(matches(., 'undated'))][1]"
          />
        </xsl:when>
        <xsl:when
          test="*:field[matches(@name, '^(copyrightDate)$')][not(matches(., 'undated'))]">
          <xsl:apply-templates
            select="*:field[matches(@name, '^(copyrightDate)$')][not(matches(., 'undated'))][1]"
          />
        </xsl:when>
        <xsl:when
          test="*:field[matches(@name, '^(dateCreated)$')][not(matches(., 'undated'))]">
          <xsl:apply-templates
            select="*:field[matches(@name, '^(dateCreated)$')][not(matches(., 'undated'))][1]"
          />
        </xsl:when>
        <xsl:when
          test="*:field[matches(@name, '^(dateValid)$')][not(matches(., 'undated'))]">
          <xsl:apply-templates
            select="*:field[matches(@name, '^(dateValid)$')][not(matches(., 'undated'))][1]"
          />
        </xsl:when>
        <xsl:when
          test="*:field[matches(@name, '^(dateCaptured)$')][not(matches(., 'undated'))]">
          <xsl:apply-templates
            select="*:field[matches(@name, '^(dateCaptured)$')][not(matches(., 'undated'))][1]"
          />
        </xsl:when>
        <xsl:when
          test="*:field[matches(@name, '^(dateModified)$')][not(matches(., 'undated'))]">
          <xsl:apply-templates
            select="*:field[matches(@name, '^(dateModified)$')][not(matches(., 'undated'))][1]"
          />
        </xsl:when>
        <xsl:when
          test="*:field[matches(@name, '^(dateOther)$')][not(matches(., 'undated'))]">
          <xsl:apply-templates
            select="*:field[matches(@name, '^(dateOther)$')][not(matches(., 'undated'))][1]"
          />
        </xsl:when>
        <xsl:when
          test="*:field[matches(@name, '^(orig_dateIssued)$')][not(matches(., 'undated'))]">
          <xsl:apply-templates
        ...
        <xsl:when
          test="*:field[matches(@name, '^(orig_dateCreated)$')][not(matches(., 'undated'))]">
          <xsl:apply-templates
            select="*:field[matches(@name, '^(orig_dateCreated)$')][not(matches(., 'undated'))][1]"
          />
        </xsl:when>
        <xsl:when
          test="*:field[matches(@name, '^(orig_dateValid)$')][not(matches(., 'undated'))]">
          <xsl:apply-templates
        ...
        <xsl:when
          test="*:field[matches(@name, '^(orig_dateOther)$')][not(matches(., 'undated'))]">
          <xsl:apply-templates
            select="*:field[matches(@name, '^(orig_dateOther)$')][not(matches(., 'undated'))][1]"
          />
        </xsl:when>
        <xsl:when
          test="*:field[matches(@name, '^(host_dateIssued)$')][not(matches(., 'undated'))]">
          <xsl:apply-templates
        ...
        <xsl:when
          test="*:field[matches(@name, '^(host_dateCreated)$')][not(matches(., 'undated'))]">
          <xsl:apply-templates
            select="*:field[matches(@name, '^(host_dateCreated)$')][not(matches(., 'undated'))][1]"
          />
        </xsl:when>
        <xsl:when
          test="*:field[matches(@name, '^(host_dateValid)$')][not(matches(., 'undated'))]">
          <xsl:apply-templates
        ...
        <xsl:when
          test="*:field[matches(@name, '^(host_dateOther)$')][not(matches(., 'undated'))]">
          <xsl:apply-templates
      ...
      <!-- Rights -->
      <xsl:choose>
        <!-- Use @valueURI on field[@name='useRestrict'][1] -->
        <xsl:when
        ...
        <!-- WHen text value starts with 'http', use text value of 
          field[@name='useRestrict'][1] -->
        <xsl:when
        ...
        <!-- Map uvaMAP useRestrict text to dcterms:description, 
          plus an empty dcterms:rights element -->
        <xsl:otherwise>
          <dcterms:description>
            <xsl:value-of select="*:field[matches(@name, '^(useRestrict)$')][1]"/>
          </dcterms:description>
          <dcterms:rights/>
        </xsl:otherwise>
      </xsl:choose>
      ...
      <xsl:choose>
        ...
        <xsl:when test="*:field[@name = 'uri'][@access = 'object in context']">
          <xsl:apply-templates
            select="*:field[@name = 'uri'][@access = 'object in context'][1]"/>
        </xsl:when>
  ...
  <!-- Creator/Contributor -->
  <xsl:template
    match="*:field[matches(@name, '^(comp_|host_|orig_|work_)?(creator|contributor)$')]">
    <xsl:variable name="creatorContributorElementName">
      <xsl:choose>
        <xsl:when test="matches(@name, 'creator')">creator</xsl:when>
        <xsl:otherwise>contributor</xsl:otherwise>
      </xsl:choose>
    </xsl:variable>
    <xsl:element name="dcterms:{$creatorContributorElementName}">
      <xsl:copy-of select="@valueURI"/>
      <xsl:value-of select="normalize-space(.)"/>
      <xsl:call-template name="createTrailingLabel"/>
    </xsl:element>
  ...
  <xsl:template
    ...
    <dcterms:created>
      <xsl:value-of
        select="replace(replace(normalize-space(.), ' TO ', '..'), '\*', '9999')"/>
    </dcterms:created>
  </xsl:template>
  ...
  <xsl:template match="*:field[matches(@name, 'physDimensions$')]">
    ...
    <xsl:if
      test="not(exists(../*:field[matches(@name, 'physExtent$') and matches(., $thisValue)]))">
      <dcterms:extent>
        <xsl:value-of select="normalize-space(.)"/>
        <xsl:call-template name="createTrailingLabel"/>
      </dcterms:extent>
