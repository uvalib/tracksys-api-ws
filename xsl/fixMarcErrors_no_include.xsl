...
-->
    <marcDesc xmlns:sch="http://purl.oclc.org/dsdl/schematron"
      ...
      </pattern> <record tag="-1" desc="Record">
  ...
  </controlfield> <controlfield tag="008" materialType="BK" desc="Book">
  ...
  <xsl:template match="*:record" mode="phase2">
    <record>
      ...
      <!-- Copy 991 not containing 'MARC validation project' -->
      <xsl:apply-templates
        select="*:datafield[@tag = '991'][not(matches(*:subfield[@code = 'a'], 'MARC validation project'))]"
        mode="phase2"/>
      <!-- Add 991 containing 'MARC validation project' -->
...
-->
  ...
  <!-- Join all 040s into a single datafield --> <!--<xsl:template match="*:datafield[@tag = '040' and following-sibling::*:datafield[@tag = '040']][1]">-->
